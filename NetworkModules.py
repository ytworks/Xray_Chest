...
def scratch_model(x, SIZE, CH, istraining, rmax, dmax, keep_probs, vname, reuse):
    Initializer = 'He'
    Activation = 'Gelu'
    ...
    # Stem
    # Batch Normalization
    with tf.variable_scope(vname, reuse=reuse):
...
def pretrain_model(x, vname='transfer_Weight_Regularization', is_save=True, reuse=False):
    p = trans.Transfer(x, 'densenet121', pooling=None, vname=vname,
                       trainable=True, is_save=is_save)
    with tf.variable_scope(vname, reuse=reuse):
